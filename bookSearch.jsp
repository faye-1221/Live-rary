...
<body>
	...
	<header>
		...
		<form action="bookSearch.jsp">
			<input type="text" name="searchValue" value="<%=(searchValue == null) ? "" : searchValue%>" placeholder="도서 이름 입력"> <input type="submit" value="검색">
		</form>
		<nav>
...
if (isAdmin || !checkOutList.contains(result.getString(2)) || limit >= 3
		|| reservationList.contains(result.getString(2)) || myCheckOutList.contains(result.getString(2)))
	out.print("disabled");%> onclick="reservation(this.id)" value="대출 예약"> <%
 /*Collections.frequency(reservationList, id);*/
 %></td>
